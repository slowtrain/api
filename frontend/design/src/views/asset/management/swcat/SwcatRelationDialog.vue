<!--
 ...
 Description : 소프트웨어 카탈로그 > 관련 카탈로그 추가 Dialog
...
-->
<template>
  <el-dialog :title="labels.swcatDialogTitle" :visible.sync="showDialogFlag" width=1200px  v-on:close="close">
    <b-row>
      <b-col sm="3">
        <Tree
          v-bind:tree="tree"
      ...
      <b-col sm="9">
        ...
        <b-row>
          <b-col sm="12">
            <p class="grid-line"></p>
            <data-tables-server ref="listGrid" :table-props='listGrid.tableProps' :data="listGrid.data" :filters="listGrid.filters" :total="listGrid.total" v-on:query-change="getListData" :pagination-props="{pageSizes: [5, 10, 15] }" @selection-change="selectCIGridRow">
  ...
  methods: {
    ...
    getListData: function (srchCnd) {
      ...
      this.$http.get(
        ...
        '&orgId=' + this.orgId + '&searchParam=' + encodeURI(searchParam) + '&treeSearchParam=' + treeSearchParam + '&swcattype=PRODUCT' + '&dialogType=relation' + '&dialogParam=' + this.param.swcatId
      ).then(res => {
