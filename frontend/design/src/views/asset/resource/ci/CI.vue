  ...
  <div class="animated fadeIn">
    <b-card>
      <b-row>
        ...
        <b-col sm="10">
          <div class="comm-btn-box" role="group">
            ...
            <b-button variant="primary" class="px-3 sm" v-show="visibleWorkflow" v-on:click="workflow">Workflow</b-button>
          </div>
        </b-col>
      </b-row>
      <p class="grid-line"></p>
      <b-row>
        <b-col sm="12">
          <!--LIST-->
          <el-tabs v-if="activeTabFlag.main" v-model="activeTab" :before-leave="chgTab" @tab-click="clickTab">
            ...
            <!--relationInfos-->
            <el-tab-pane  name="cirelationInfos" :label='labels.relationInfos' v-if="(activeTab !== 'list')">
              <CIRelationInfos
                ref="ciRelationInfos"
                v-if="activeTabFlag.relationInfos"
...
import CIRelationInfos from './CIRelationInfos'
...
export default {
  ...
  components: {
    'Tree': Tree,
    'CIDetail': CIDetail,
    'CIRelationInfos': CIRelationInfos,
    ...
    return {
      ...
      relationSwGrid: {
        ...
        titles: [
          ...
          { prop: 'detailedversion', label: 'detailedversion', width: '160px' },
          { prop: 'discoverypath', label: 'discoverypath', width: '160px' },
  ...
  watch: {
    'listGrid.currentData.wfAccessible': function (val) {
      if (val) {
        this.wfAccessible = true
      } else {
        this.wfAccessible = false
      }
    },
    '$route': function () {
    ...
    activeTab: function (val) {
      ...
      } else if (val === 'cirelationInfos') {
        this.activeTabFlag.relationInfos = true
      } else if (val === 'cirelation') {
  ...
  computed: {
    visibleWorkflow: function () {
      // && this.listGrid.currentData.wfAccessible
      return this.authorities.CI_SAVE && (this.activeTab !== 'list') && this.wfAccessible
    },
    visibleAdd: function () {
  ...
  methods: {
    workflow: function () {
      this.$util.workflow(this, this.$options.objectId, this.listGrid.currentData.id, this.getRowData)
    },
    clickTab: function (tab, event) {
      let id = this.listGrid.currentData.id
      if (tab.name === 'cirelationInfos') {
    ...
    getInitData: function () {
      this.$http.get(
        ...
        if (!this.$util.isNull(this.$route.params.cl)) {
          this.tree.expandKeys = [this.$route.params.ancestorId]
        }
    ...
    getRowData: function (id) {
      this.$http.get(
        ...
        this.wfAccessible = false
        this.wfAccessible = res.data.wfAccessible
        this.wfInstance = {}
        this.wfInstance = res.data.wfInstance
      }).catch(error => {
        console.log(error)
      })
      this.getSpecData(id)
      ...
      if (activeTab === 'list') {
        ...
        if (rtnFlag) {
          ...
          this.$nextTick(function () {
            this.activeTabFlag.relationInfos = false
    ...
    clickAddBtn: function (tree) {
      this.$http.get(
        ...
        this.relationGrid.data = []
        this.$http.get(
          '/episode/api/asset/resource/ci/ciextnm?classstructureId=' + tree.classstructureId + '&lang=' + this.$store.getters.getUser.lang
        ).then(res => {
