    ...
    pwSave: function () {
      ...
      if (validate) {
        this.$http.put(
          ...
          this.$swal({type: 'success', title: this.labels.saveMessage})
          this.dialogFormVisible = false
        }).catch(error => {
          this.$swal({type: 'warning', title: this.labels.failMessage})
          console.log(error)
        })
      }
    },
      ...
      if (this.isValidationCheck()) {
        if (this.listGrid.currentData.state === 'CHANGED' || this.listGrid.currentData.state === 'ADDED') {
          if (this.listGrid.currentData.hasOwnProperty('id')) {
            this.$http.put('/episode/api/system/user/' + this.listGrid.currentData.id, this.listGrid.currentData).then(res => {
              this.getRowData(res.data.id)
              this.getListData()
              this.$swal({type: 'success', title: this.labels.saveMessage, animation: false})
            })
          } else {
            this.$http.put('/episode/api/system/user', this.listGrid.currentData).then(res => {
    ...
    del: function () {
      if (this.listGrid.currentData.hasOwnProperty('id')) {
        ...
        if (this.filterGrid.data.filter(filter => filter.username === username).length > 0) {
          this.$swal({type: 'warning', title: this.labels.userfitlerMessage, animation: true})
          return
        }
        if (this.authorityGrid.data.filter(auth => auth.username === username).length > 0) {
          this.$swal({type: 'warning', title: this.labels.authMessage, animation: true})
          return
        }
        this.$http.delete('/episode/api/system/user/' + id).then(() => {
      ...
      this.listGrid.currentData = {}
      this.activeTab = 'list'
      this.$swal({type: 'success', title: this.labels.deleteMessage})
    },
    // excel download
